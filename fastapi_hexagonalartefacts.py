...
DATABASE_URL = "postgresql+psycopg2://awasekhirnisyed:YOUR_PASSWORD_GOESHERE@localhost:5432/profxdb"
PROJECT_DIR = "project"
...
def generate_controller(table_name: str, class_name: str) -> str:
    """Generate controller for a table"""
...
def initialize_controller(service: IAsync{class_name}Service):
    ...
    async def create_{table_name}(data: {class_name}Create):
    ...
    async def get_all_{table_name}s(skip: int = 0, limit: int = 100):
    ...
    async def get_{table_name}_by_id(id: str):
    ...
    async def update_{table_name}(id: str, data: {class_name}Update):
        \"\"\"Update {table_name}\"\"\"
        try:
            validated_data = validate_{table_name}_update(data)
            entity = {table_name}_mapper.to_entity(validated_data)
            result = await service.update(id, entity)
            if not result:
                raise HTTPException(status_code=404, detail="{class_name} not found")
            return {table_name}_mapper.to_dto(result)
        except Exception as e:
    ...
    async def delete_{table_name}(id: str):
        \"\"\"Delete {table_name}\"\"\"
        try:
...
def generate_validation_schemas(table_name: str, class_name: str) -> str:
    ...
    return f"""from pydantic import BaseModel, validator
...
def generate_namespace(controllers: List[str]) -> str:
    ...
    imports = "\n".join([f"from api.controllers.{controller}_controller import initialize_controller as {controller}_router"
                        for controller in controllers])
    router_registrations = "\n".join([f"api_router.include_router({controller}_router({controller.capitalize()}Service()))"
                                    for controller in controllers])
